...
{
 "use strict";
 var encrypt2,Client,WebSharper,Obj,UI,Var$1,Operators,HtmlModule,attr,Var,EventTarget,WindowOrWorkerGlobalScope,JavaScript,JS,ConcreteVar,Snap,List,Strings,AttrProxy,Doc,Pervasives,Seq,T,Arrays,Char,Client$1,Templates,Abbrev,Fresh,SC$1,Attrs,DomUtility,View,AttrModule,Enumerator,T$1,Object,Node,Collections,Dictionary,Unchecked,Numeric,BindVar,SC$2,HashSet,Docs,SC$3,Random,Elt,Array,DocElemNode,CharacterData,SC$4,DictionaryUtil,Element,Prepare,Slice,KeyCollection,An,Settings,Mailbox,Updates,Attrs$1,Dyn,String,CheckedInput,Docs$1,RunState,NodeSet,Concurrency,Anims,SC$5,DynamicAttrNode,DateUtil,SC$6,SC$7,AppendList,HashSetUtil,Error,KeyNotFoundException,Scheduler,Easing,AsyncBody,SC$8,CT,HashSet$1,CancellationTokenSource,DomNodes,OperationCanceledException,Lazy,SC$9,Queue,LazyExtensionsProxy,LazyRecord,String$1,Runtime,console,Math,Date;
 encrypt2=Global.encrypt2=Global.encrypt2||{};
 Client=encrypt2.Client=encrypt2.Client||{};
 ...
 SC$1=Global.StartupCode$encrypt2$Client=Global.StartupCode$encrypt2$Client||{};
 ...
 {
  var plainText,encryptedText,decryptedText,encryptionKey,_;
  plainText=Var$1.Create$1("");
  encryptedText=Var$1.Create$1("");
  decryptedText=Var$1.Create$1("");
  encryptionKey=Var$1.Create$1(Client.displayKey());
  _=Doc.Element("div",[AttrProxy.Create("class","auth-container")],[Doc.Element("h1",[],[Doc.TextNode("Encryption App")]),Doc.Element("div",[AttrProxy.Create("class","form-group")],[Doc.Element("h3",[],[Doc.TextNode("Encrypt a Message")]),Doc.Input([AttrProxy.Create("class","form-control")],plainText),Doc.Element("button",[AttrProxy.Create("class","btn btn-default"),AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    return encryptedText.Set(Client.encrypt(plainText.Get()));
   };
  })],[Doc.TextNode("Encrypt")]),Doc.Element("button",[AttrProxy.Create("class","btn btn-default"),AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    return encryptedText.Set(Client.reverseEncrypt(plainText.Get()));
   };
  })],[Doc.TextNode("Reverse Encrypt")]),Doc.Element("button",[AttrProxy.Create("class","btn btn-default"),AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    return encryptedText.Set(Client.caseSensitiveEncrypt(plainText.Get()));
   };
  })],[Doc.TextNode("Case-Sensitive Encrypt")])]),Doc.Element("div",[AttrProxy.Create("class","form-group")],[Doc.Element("h3",[],[Doc.TextNode("Encrypted Message")]),Doc.Input([AttrProxy.Create("class","form-control"),AttrProxy.Create("readonly","readonly")],encryptedText)]),Doc.Element("div",[AttrProxy.Create("class","form-group")],[Doc.Element("h3",[],[Doc.TextNode("Decrypt the Message")]),Doc.Input([AttrProxy.Create("class","form-control")],encryptedText),Doc.Element("button",[AttrProxy.Create("class","btn btn-default"),AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    return decryptedText.Set(Client.decrypt(encryptedText.Get()));
   };
  })],[Doc.TextNode("Decrypt")])]),Doc.Element("div",[AttrProxy.Create("class","form-group")],[Doc.Element("h3",[],[Doc.TextNode("Decrypted Message")]),Doc.Input([AttrProxy.Create("class","form-control"),AttrProxy.Create("readonly","readonly")],decryptedText)]),Doc.Element("div",[AttrProxy.Create("class","form-group")],[Doc.Element("h3",[],[Doc.TextNode("Encryption Key")]),Doc.Input([AttrProxy.Create("class","form-control"),AttrProxy.Create("readonly","readonly")],encryptionKey),Doc.Element("button",[AttrProxy.Create("class","btn btn-default"),AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    return encryptionKey.Set(Client.saveKey());
   };
  })],[Doc.TextNode("Save Key")]),Doc.Element("button",[AttrProxy.Create("class","btn btn-default"),AttrProxy.HandlerImpl("click",function()
  {
   return function()
   {
    var key;
    key=self.prompt("Enter the encryption key:");
    return!Strings.IsNullOrWhiteSpace(key)?(Client.loadKey(key),encryptionKey.Set(Client.displayKey())):null;
   };
  })],[Doc.TextNode("Load Key")])])]);
  Templates.LoadLocalTemplates("");
  Doc.RunById("main",_);
 };
 Client.displayKey=function()
 {
  return Strings.concat("",List.map(Global.id,Client.keyList()));
 };
 Client.encrypt=function(plainText)
 {
  return Strings.concat("",Arrays.map(Global.id,Arrays.ofSeq(Seq.map(function(letter)
  {
   var index;
   index=Seq.findIndex(function(y)
   {
    return letter===y;
   },Client.charsList());
   return Client.keyList().get_Item(index);
  },plainText))));
 };
 Client.reverseEncrypt=function(plainText)
 {
  return Client.encrypt(Arrays.ofSeq(Seq.rev(plainText)).join(""));
 };
 Client.caseSensitiveEncrypt=function(plainText)
 {
  return Strings.concat("",Arrays.map(Global.id,Arrays.ofSeq(Seq.map(function(letter)
  {
   var lower,index,index$1;
   return Char.IsUpper(letter)?(lower=String$1.fromCharCode(letter).toLowerCase().charCodeAt(0),(index=Seq.findIndex(function(y)
   {
    return lower===y;
   },Client.charsList()),String$1.fromCharCode(Client.keyList().get_Item(index)).toUpperCase().charCodeAt(0))):(index$1=Seq.findIndex(function(y)
   {
    return letter===y;
   },Client.charsList()),Client.keyList().get_Item(index$1));
  },plainText))));
 };
 Client.decrypt=function(cipherText)
 {
  return Strings.concat("",Arrays.map(Global.id,Arrays.ofSeq(Seq.map(function(letter)
  {
   var index;
   ...
   return Client.charsList().get_Item(index);
  },cipherText))));
 };
 Client.saveKey=function()
 {
  return Client.displayKey();
 };
 Client.loadKey=function(key)
 {
  var t;
  function f(cl,kl)
  {
   return function(t$1)
   {
    return[List.append(cl,List.ofArray([t$1[0]])),List.append(kl,List.ofArray([t$1[1]]))];
   };
  }
  t=Seq.fold(function($1,$2)
  {
   return(function($3)
   {
    return f($3[0],$3[1]);
   }($1))($2);
  },[T.Empty,T.Empty],List.mapi(function(i,c)
  {
   return[Client.charsList().get_Item(i),c];
  },List.ofSeq(Seq.map(Global.id,key))));
  Client.set_charsList(t[0]);
  Client.set_keyList(t[1]);
 };
 Client.keyList=function()
 {
  SC$1.$cctor();
  return SC$1.keyList;
 };
 Client.charsList=function()
 {
  SC$1.$cctor();
  return SC$1.charsList;
 };
 Client.set_charsList=function($1)
 {
  SC$1.$cctor();
  SC$1.charsList=$1;
 };
 Client.set_keyList=function($1)
 {
  SC$1.$cctor();
  SC$1.keyList=$1;
 };
 Client.shuffle=function(list)
 {
  var rng,array,i,$1,j,temp;
  rng=new Random.New();
  array=Arrays.ofList(list);
  for(i=0,$1=Arrays.length(array)-1;i<=$1;i++){
   j=rng.Next$1(Arrays.length(array));
   temp=Arrays.get(array,i);
   Arrays.set(array,i,Arrays.get(array,j));
   Arrays.set(array,j,temp);
  }
 ...
 SC$1.$cctor=function()
 {
  ...
  SC$1.charsList=List.ofSeq(Seq.delay(function()
  {
   return Seq.map(String$1.fromCharCode,Operators.range(32,126));
  }));
  SC$1.keyList=Client.shuffle(Client.charsList());
 };
